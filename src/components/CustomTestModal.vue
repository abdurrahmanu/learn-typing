<template>
    <div v-if="customTestModal && route.name === 'home'" class="max-w-[500px] w-[100%] fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] rounded-md py-5 max-h-[calc(100dvh_-_20px)]" :class="[theme === 'dark' ? 'bg-neutral-800' : 'bg-slate-300 shadow-sm shadow-slate-500']">
        ...
        <div>            
            <div v-if="!saveCustomText" class="space-y-1 text-center">
                ...
                <div @click="startSavingCustomText" class="py-[2px] m-auto text-center border rounded-full px-4 w-fit border-slate-600 hover:bg-green-500 hover:text-white text-xs font-medium">ADD TEST</div>
            ...
            <div v-else class="pt-3 text-center">
                ...
                <div v-else class="space-y-4">                    
                    <input name="custom-test-title" v-model="customTextTitle" type="text" :placeholder="titlePlaceholder" class="p-1 text-center bg-transparent border-b outline-none border-b-blue-500 placeholder:text-center">
                    <div class="flex justify-center gap-2 m-auto text-xs font-medium w-fit">                    
                        ...
                        <div @click="saveNewCustomText" class="py-[2px] m-auto text-center border rounded-r-full px-4 w-fit border-slate-600 hover:bg-green-500 hover:text-white">SAVE</div>
                    </div>
                </div>
            </div>
        </div>
