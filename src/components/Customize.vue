<template>
  <div ref="containerEl" :class="[!hasCompletedSession && !alphabetsMode_ ? 'min-[1350px]:right-[140px]' : '', isMobileOS() && focus ? 'hidden' : 'block']" class="py-1 m-auto rounded-md w-fit relative max-w-[90%] bg-inherit font-mono">
    <div :style="{'width': completionLevel + '%'}" :class="[theme === 'dark' ? 'bg-green-400 after:bg-green-400' : 'bg-green-600 after:bg-green-600', !completionLevel || playerInput.length < 2 ? 'after:w-0 after:hidden' : 'after:w-[3%] after:right-0 after:absolute after:top-0 after:bottom-0 after:blur-[5px]']" class="absolute left-0 bottom-0 top-0 m-auto rounded-md max-[460px]:hidden" v-if="blind" ></div>
    ...
    <div v-if="!hasCompletedSession && alphabetsMode_" :class="[appTheme]" class="rounded-md w-[90%] min-w-fit m-auto max-w-fit ring-1 ring-green-600 min-[1350px]:ring-0 relative py-3 px-10 font-normal text-[12px] group">
      <div class="flex flex-wrap items-center justify-center gap-[8px] rounded-md min-w-fit m-auto max-w-fit hover:ring-[1px] hover:ring-slate-500 group p-[2px] relative">        
        ...
        <div class="absolute left-0 text-black top-[115%] shadow-sm shadow-slate-500 px-[6px] bg-neutral-100 rounded-full whitespace-nowrap group-hover:block hidden">format</div>
      </div>
    </div>
  </div>
</template>
