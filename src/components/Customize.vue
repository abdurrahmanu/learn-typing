<template>
  <div class="p-[2px] m-auto rounded-md w-fit relative max-w-[90%] xl:right-[130px]">
    <div :style="{'width': completionLevel + '%'}" :class="[theme === 'dark' ? 'bg-blue-200' : 'bg-green-500']" class="absolute left-0 bottom-0 top-0 m-auto rounded-md"></div>
    ...
    <div v-if="!hasCompletedSession && alphabetsMode_" :class="[appTheme]" class="rounded-md  w-[90%] min-w-fit m-auto max-w-fit ring-1 ring-blue-600 xl:ring-0 relative xl:z-[3] flex gap-5 py-2 px-3 font-normal text-[12px] flex-wrap justify-center">
          <div :class="[alphabetsConfig.uppercase ? 'border-green-500 text-green-500' : 'border-slate-600']" @click="changeMode('uppercase')" class="px-[5px] hover:scale-105 rounded-md"  >uppercase</div>
          <div :class="[alphabetsConfig.customCase ? 'border-green-500 text-green-500' : 'border-slate-600']" @click="changeMode('customCase')" class="px-[5px] hover:scale-105 rounded-md">randomcase</div>
          <div :class="[alphabetsConfig.spaced ? 'border-green-500 text-green-500' : 'border-slate-600']" @click="changeMode('spaced')" class="px-[5px] hover:scale-105 rounded-md"  >space</div>
          <div :class="[alphabetsConfig.backwards ? 'border-green-500 text-green-500' : 'border-slate-600']" @click="changeMode('backwards')" class="px-[5px] hover:scale-105 rounded-md"  >reverse</div>
          <div :class="[alphabetsConfig.random ? 'border-green-500 text-green-500' : 'border-slate-600']" @click="changeMode('random')" class="px-[5px] hover:scale-105 rounded-md"  >random</div>
      </div>
  </div>
</template>
