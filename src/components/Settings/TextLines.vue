<template>
    <div v-if="!isMobile()" :class="[theme === 'dark' ? 'hover:bg-neutral-800' : 'hover:bg-zinc-200']" class="py-4 pl-5 gap-x-3 gap-y-1 font-[400]">
        ...
        <div class="flex flex-wrap gap-x-2 gap-y-1">
            ...
            <div @click="textLines = 4" class="flex px-5 py-1 border rounded-md" :class="[textLines === 4 ? 'border-green-500' : 'border-slate-500']" v-if="!isMobile()">
                <p class="w-fit ">4</p>
            </div>
            <div @click="textLines = 5" class="flex px-5 py-1 border rounded-md" :class="[textLines === 5 ? 'border-green-500' : 'border-slate-500']" v-if="!isMobile()">
                <p class="w-fit ">5</p>
            </div>
        </div>
    </div>
