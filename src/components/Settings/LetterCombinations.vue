    ...
    <div :class="[theme === 'dark' ? 'hover:bg-neutral-800' : 'hover:bg-zinc-200']" class="py-1 pl-5">
        ...
        <div class="relative m-auto h-fit w-fit">            
            <div v-for="(line, index) in shiftKey ? entries[0] : entries[1]" class="flex gap-1 justify-center z-[3]">
                <div @click="addSelection(entry)"  v-for="(entry, index) in line" :key="index" :class="[mixCharactersArray.includes(entry) ? ' border-blue-700' : 'border-neutral-600'] " class="py-[1px] px-2 rounded-md border text-base inline-block min-w-[20px] text-center m-[3px] font-medium">{{ entry }}</div>
            </div>
        </div>
    </div>
